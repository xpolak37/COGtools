...
def consensus(organism_name, em_file, om_file, batch_file, fasta_file, get_pseudo=False, get_ncrna=False, gff_file=None,
              cat_choice=1, output_dir=os.getcwd()):
    """
            ...
            # two tools match in assignment
                ...
                if idxs[0] == [1,2] or idxs[0] == [0,2]:
                    ...
                    try:
                        ...
                        df["attribute"][row] = df["attribute"][row].replace(search('CAT=(.*);', df["attribute"][row]).group(0), "CAT=" + cat + ";")
    ...
    # categories choice
    if cat_choice == 1:
        df['attribute'] = df['attribute'].apply(lambda x: x.replace(search('CAT=(.*);', x).group(1), search('CAT=(.*);', x).group(1)[0]))
    elif cat_choice == 2:
        df['attribute'] = df['attribute'].apply(lambda x:  x.replace(search('CAT=(.*);', x).group(1), search('CAT=(.*);', x).group(1)
                                                [randint(0, len(search('CAT=(.*);', x).group(1))-1)]) if len(search('CAT=(.*);', x).group(1)) > 1 else x)
    else:
...
def consensus_draft(organism_name, proteins, em_file, om_file, batch_file, cat_choice=1, output_dir=os.getcwd()):
    """
